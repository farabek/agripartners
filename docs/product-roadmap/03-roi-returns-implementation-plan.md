...
This document defines exact implementation steps for replacing the hard-coded `20%` ROI assumption with deal-level projected ROI.
...
This is planning only. It does not change code, migrations, commits, pushes, smart contracts, or deployed environments.

## Recommended MVP Scope

Implement ROI & Returns as an off-chain, database-backed reporting feature.
...
Add deal-level projected ROI while preserving existing behavior for all current deals.
...
2. Update `schema.sql` so fresh database setup includes `projected_roi_pct`.

3. Keep the default `20` so existing deals behave exactly as they do today.

4. Do not modify smart contract deployment arguments in this phase.

### Risk level

Low.

The migration is additive and preserves current behavior through the default value.

### Rollback strategy

- Code rollback: keep service code able to fall back to `20` if the column is unavailable.
- DB rollback, if required in a non-production environment:
...
Avoid dropping the column in production unless no code path depends on it.

### Testing strategy

- Run migrations on a local or staging database.
...
Move ROI calculation out of the hard-coded constant and into deal-level data.

### Files affected

- `backend/src/services/dealService.js`
- Backend service tests, if present or newly added
...
The calculation is central to investor-facing and admin-facing financial summaries. The risk is mostly formatting, unit handling, and backward compatibility.

### Rollback strategy

- Revert service changes and keep using the existing `ROI_PERCENT = 20`.
- Keep the DB column in place because it is additive and harmless.
...
- `backend/src/services/dealService.js`
- Backend route tests, if present or newly added
...
The main risks are authorization gaps and inconsistent response shapes across investor and admin routes.
...
The biggest risk is user misunderstanding. Technical risk is limited if response compatibility fields remain.
...
Admin UI changes affect operational workflows and financial reporting. The main risks are mistaken return entry, unclear ledger status, and ROI edits after investor-facing materials were shown.
