...
Этот документ аудирует, как ROI summary data должна быть показана в Investor Portal и Admin Dashboard.
...
Это read-only design audit. Он не меняет code, API routes, frontend UI, commits или deployment state.

## Findings

### Existing API responses already containing ROI fields

Текущий backend уже отдает ROI summary fields в одном investor response:
...
Admin return creation endpoint также возвращает summary после записи return:
...
Investor returns endpoint отдает ledger rows:
...
Следующие responses сейчас не содержат ROI summary fields:
...
- Admin routes currently do not provide:
...
Existing Investor Portal уже может render current summary with existing fields, но безопасному MVP UI нужны explicit fields:
...
Investor detail уже implemented в `frontend/app.js`:
...
Current investor detail bundle fetches:
...
В `frontend/app.js` есть two admin experiences:
...
3. Add compact MVP disclaimer near `Investment Summary` or `Returns History`:
...
2. Add admin return summary panel to real admin deal detail:
...
3. Add admin return ledger below the record form:
