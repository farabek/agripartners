# Documentation Inventory

This inventory covers the current documentation tree under `docs/`. It is grouped by workstream, not by proposed future structure.

## Summary Counts

| Group | File Count |
| --- | ---: |
| Core Project Docs | 11 |
| Investor Package | 25 |
| NEAR Outreach | 11 |
| NEAR Ecosystem | 4 |
| NEAR Outreach Toolkit | 11 |
| Screenshots | 18 |
| Other | 30 |
| **Total** | **110** |

## Core Project Docs

These files introduce the product, role portals, and current NEAR Testnet demo.

- `docs/README.md`
- `docs/README-ru.md`
- `docs/investor-portal.md`
- `docs/investor-portal-ru.md`
- `docs/farmer-portal.md`
- `docs/farmer-portal-ru.md`
- `docs/admin-dashboard.md`
- `docs/admin-dashboard-ru.md`
- `docs/near-testnet.md`
- `docs/near-testnet-ru.md`
- `docs/contacts.md`

## Investor Package

These files support investor storytelling, demo readiness, deck preparation, scripts, and pilot deal explanation.

- `docs/investor-package/AgriPartners_Investor_Deck_v1_EN.pptx`
- `docs/investor-package/AgriPartners_Investor_Deck_v1_RU.pptx`
- `docs/investor-package/demo-day-script-en.md`
- `docs/investor-package/demo-day-script-ru.md`
- `docs/archive/investor/demo-script-v1.md`
- `docs/archive/investor/demo-script-v1-ru.md`
- `docs/investor-package/demo-video-script-v2.md`
- `docs/investor-package/investor-one-pager-en-v2.md`
- `docs/investor-package/investor-one-pager-ru-v2.md`
- `docs/investor-package/investor-readiness-review.md`
- `docs/investor-package/one-pager-v1.md`
- `docs/investor-package/one-pager-v1-ru.md`
- `docs/investor-package/pilot-deals-summary.md`
- `docs/investor-package/pilot-deals-summary-ru.md`
- `docs/archive/investor/pitch-deck-v1.md`
- `docs/archive/investor/pitch-deck-v1-ru.md`
- `docs/investor-package/pitch-deck-v2.md`
- `docs/investor-package/pitch-deck-v2-ru.md`
- `docs/investor-package/presentation-assets-plan.md`
- `docs/investor-package/presentation-outline-en.md`
- `docs/investor-package/presentation-outline-ru.md`
- `docs/investor-package/screenshot-capture-guide.md`
- `docs/investor-package/screenshot-checklist.md`
- `docs/investor-package/screenshot-checklist-ru.md`
- `docs/investor-package/screenshots/README.md`

## NEAR Outreach

These files explain AgriPartners for NEAR ecosystem conversations.

- `docs/near-outreach/README.md`
- `docs/near-outreach/project-summary-en.md`
- `docs/near-outreach/project-summary-ru.md`
- `docs/near-outreach/founder-introduction-en.md`
- `docs/near-outreach/founder-introduction-ru.md`
- `docs/near-outreach/elevator-pitch-en.md`
- `docs/near-outreach/elevator-pitch-ru.md`
- `docs/near-outreach/near-use-case-en.md`
- `docs/near-outreach/near-use-case-ru.md`
- `docs/near-outreach/ecosystem-one-pager-en.md`
- `docs/near-outreach/ecosystem-one-pager-ru.md`

## NEAR Ecosystem

These files organize the NEAR ecosystem planning track.

- `docs/near-ecosystem/README.md`
- `docs/near-ecosystem/near-ecosystem-map.md`
- `docs/near-ecosystem/near-engagement-plan.md`
- `docs/near-ecosystem/near-priority-targets.md`

## NEAR Outreach Toolkit

These files provide reusable outreach message templates.

- `docs/near-outreach-toolkit/README.md`
- `docs/near-outreach-toolkit/intro-message-en.md`
- `docs/near-outreach-toolkit/intro-message-ru.md`
- `docs/near-outreach-toolkit/followup-message-en.md`
- `docs/near-outreach-toolkit/followup-message-ru.md`
- `docs/near-outreach-toolkit/project-introduction-email-en.md`
- `docs/near-outreach-toolkit/project-introduction-email-ru.md`
- `docs/near-outreach-toolkit/meeting-request-en.md`
- `docs/near-outreach-toolkit/meeting-request-ru.md`
- `docs/near-outreach-toolkit/feedback-request-en.md`
- `docs/near-outreach-toolkit/feedback-request-ru.md`

## Screenshots

These image files are referenced by root role documentation and investor package planning docs.

### Investor Screenshots

- `docs/screenshots/investor/01-investor-header.png`
- `docs/screenshots/investor/02-investor-profile.png`
- `docs/screenshots/investor/03-investment-summary.png`
- `docs/screenshots/investor/04-featured-pilot-deals.png`
- `docs/screenshots/investor/05-active-hissar.png`
- `docs/screenshots/investor/06-completed-fidlot.png`

### Farmer Screenshots

- `docs/screenshots/farmer/01-farmer-profile.png`
- `docs/screenshots/farmer/02-farmer-metrics.png`
- `docs/screenshots/farmer/03-fidlot-card.png`
- `docs/screenshots/farmer/04-farmer-deals.png`

### Admin Screenshots

- `docs/screenshots/admin/01-admin-dashboard.png`
- `docs/screenshots/admin/02-admin-pilot-deals.png`
- `docs/screenshots/admin/03-admin-fidlot-profile.png`
- `docs/screenshots/admin/04-admin-fidlot-reporting.png`
- `docs/screenshots/admin/05-admin-fidlot-event-history.png`
- `docs/screenshots/admin/06-admin-hissar-profile.png`
- `docs/screenshots/admin/07-admin-hissar-reporting.png`
- `docs/screenshots/admin/08-admin-hissar-event-history.png`

## Other

These files are either legacy outputs, forum/grant materials, PDFs, HTML exports, historical planning specs, or supporting contract documents.

### Root-Level Other Files

- `docs/near-grant-application.md`
- `docs/near-grant-application-ru.md`
- `docs/near-forum-post.md`
- `docs/near-forum-post-ru.md`
- `docs/near-forum-post.html`
- `docs/one-pager.html`
- `docs/pitch-deck.html`
- `docs/funding-strategy.html`
- `docs/archive/checklist.html`
- `docs/archive/deprecated/AgriPartners-Comparison-AllSplits.pdf`
- `docs/archive/deprecated/AgriPartners-Escrow-Analysis.pdf`

### 60-40 financial models

- `docs/60-40/pdf/ru/Agri-Farmer-Fidlot-v5.9-6040-RU.pdf`
- `docs/60-40/pdf/ru/Agri-Farmer-VariantB-v2.1-6040-RU.pdf`
- `docs/60-40/pdf/ru/Agri-Investor-Fidlot-v5.9-6040-RU.pdf`
- `docs/60-40/pdf/ru/Agri-Investor-VariantB-v2.1-6040-RU.pdf`
- Matching English PDFs are in `docs/60-40/pdf/en/`.
- Editable RU/EN DOCX sources are in `docs/60-40/source/{ru,en}/`.

### Superpowers Plans And Specs

- `docs/archive/deprecated/2026-05-23-smart-contract.md`
- `docs/archive/deprecated/2026-05-24-backend.md`
- `docs/archive/deprecated/2026-05-25-frontend-dashboard.md`
- `docs/archive/deprecated/2026-05-25-pitch-launch.md`
- `docs/archive/deprecated/2026-05-25-postgres-railway.md`
- `docs/archive/deprecated/2026-05-25-testnet-deploy.md`
- `docs/archive/deprecated/2026-05-26-personal-cabinet.md`
- `docs/archive/deprecated/2026-05-23-smart-contract-design.md`
- `docs/archive/deprecated/2026-05-24-backend-design.md`
- `docs/archive/deprecated/2026-05-25-frontend-design.md`
- `docs/archive/deprecated/2026-05-25-pitch-launch-design.md`
- `docs/archive/deprecated/2026-05-25-postgres-railway-design.md`
- `docs/archive/deprecated/2026-05-25-testnet-deploy-design.md`

## Empty Placeholder Folders

These folders currently exist but contain no files:

- `docs/architecture/`
- `docs/business-model/`
- `docs/demo-guide/`
- `docs/investor-deck/`
- `docs/workflows/`
