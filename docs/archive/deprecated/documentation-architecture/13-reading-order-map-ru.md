...
Эта карта определяет entry points, recommended reading order, optional documents и dependencies для каждой documentation area.

## Entire Documentation Set

### First Document

Read `docs/README.md` first. Он задает product, roles, pilot MVP architecture и основные documentation entry points.

### Second Document

Read `docs/near-outreach/project-summary-en.md` second для concise external-facing explanation of AgriPartners.
