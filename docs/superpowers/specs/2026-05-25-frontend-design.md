...
- Header: "AgriPartners" title + subtitle "Agricultural investments on NEAR Protocol"
- Cards grid: one card per deal
- Each card shows: deal type badge (Fidlot/Hissar), status badge, farmer address, investor address, total cycles × cycle duration, investment amount in NEAR, "Open →" button

**Empty state:** "No deals" message when array is empty.
...
- Header: "← Back" link, deal type, status badge, current cycle number
- Two-column section:
...
- "Refresh" button — re-fetches `/status` and `/balances` only (parameters don't change)
...
| --- | --- |
| Backend unreachable | Red error banner: "Backend unavailable" |
| Blockchain RPC fails | Status and balances show "—", DB data still visible |
| Deal not found (404) | "Deal not found" message |
| Empty deals list | "No deals" message |
